...
**Booztory** is a fully on-chain digital spotlight platform on **Base**. Creators rent 15-minute featured content slots by minting an ERC-721 token for **1 USDC**. All slot data is stored on-chain. Donations are pulled by the contract (95% forwarded to creator, 5% fee kept). No database — all reads go through wagmi `useReadContract`.

**Stack:** Next.js 16.1.6 · React 19 · TypeScript · Tailwind CSS · wagmi v2 + RainbowKit · viem · NextAuth 4 (SIWE) · Hardhat 2.28.6 · Base chain (8453)
...
**Env vars:**
- `NEXT_PUBLIC_BOOZTORY_ADDRESS` — deployed contract address
...
- `topbar.tsx` — Fixed top bar with "Booztory" wordmark (Inter font), wallet connect, FAQ link
- `pageTopbar.tsx` — Sub-page header with customizable title
...
- `historyCard.tsx` — Card for expired content. Display name = `content.authorName` → ENS/Basename of submitter.
- `upcomingCard.tsx` — Card for queued content. Same display name priority as `historyCard`.
- `icon.tsx` — Platform icon helper

### `components/modals/`
- `submitContent.tsx` — Full submission drawer: URL validation, TikTok short URL resolution, content preview, approve + mintSlot flow, mobile keyboard detection
...
### `components/wallet/`
- `connectWallet.tsx` — RainbowKit connect button with auto SIWE sign-in via `useAccountEffect`. Resolves ENS (mainnet) and Basename (Base L2). Displays basename → ENS → truncated address.

### `components/embeds/`
- `youtubeEmbed.tsx`, `tiktokEmbed.tsx` (iframe player/v1), `twitterEmbed.tsx` (react-tweet), `vimeoEmbed.tsx`, `spotifyEmbed.tsx`

### `components/tweet/` and `components/ui/`
- Custom tweet rendering components; Shadcn/UI + Radix UI — both unchanged/unmodified.
...
Both are composed in `app/layout.tsx`.
...
| File | Purpose |
|---|---|
| `lib/contract.ts` | `BOOZTORY_ADDRESS`, `USDC_ADDRESS`, `BOOZTORY_ABI`, `ERC20_ABI`, `ContentItem` type, `OnChainSlot` type, `parseSlot()`, `getPlaceholderContent()` |
| `lib/wagmi.ts` | wagmiConfig — chains: Base + mainnet (mainnet needed for ENS resolution) |
...
| `lib/youtubeMetadata.ts` | YouTube oEmbed API with 8 URL regex patterns |
...
| `hooks/usePayment.tsx` | `mintSlot(slotData)` — USDC `approve` → Booztory `mintSlot`, waits for receipts, shows confetti |
...
|---|---|
| `mintSlot(url, type, ratio, title, author, imageUrl)` | Pay slotPrice → mint ERC-721 slot token. Fees held in contract. |
| `donate(tokenId, amount)` | Pull full amount, forward 95% to creator, keep 5% fee in contract |
| `withdraw()` | Owner withdraws all accumulated fees (ETH or current paymentToken) |
...
1. `USDC.approve(BOOZTORY_ADDRESS, slotPrice)`
2. `Booztory.mintSlot(url, type, ratio, title, author, imageUrl)`
...
## Authentication Flow (wagmi + SIWE + NextAuth)

1. Wallet connects → `useAccountEffect.onConnect` fires
2. **GET /api/nonce** → UUID nonce stored in httpOnly cookie
3. Frontend builds `SiweMessage` (domain, address, chainId: 8453, nonce)
...
8. Disconnect: `signOut()` + wagmi `disconnect()`
...
- [ ] **More platforms**: Twitch, Instagram, custom uploads — mentioned in FAQ, not implemented
...
| Area | Status |
|---|---|
| ERC-721 slot contract (Booztory.sol) | Written, compiled |
...
| Content submission (approve + mintSlot) | Implemented |
| Donation flow (approve + donate) | Implemented |
| YouTube / TikTok / Twitter / Vimeo / Spotify embeds | Working |
| History & Upcoming pages | Working |
| FAQ page | Working |
| Contract deployment (Base Sepolia) | Pending — set NEXT_PUBLIC_BOOZTORY_ADDRESS after deploy |
