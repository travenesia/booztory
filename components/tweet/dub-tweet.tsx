...
import { Heart, Message } from "./icons/index" // Updated import path
    ...
    <div className="break-inside-avoid rounded-lg border border-gray-300 bg-white/20 bg-clip-padding p-6 pb-4 backdrop-blur-lg backdrop-filter">
      ...
      <div>
        ...
        {tweet.in_reply_to_status_id_str && tweet.in_reply_to_screen_name && (
          <div className="mt-5 text-base text-gray-500">
            Replying to{" "}
            <a className="text-[#1da1f2] no-underline" href={tweet.in_reply_to_url} target="_blank" rel="noreferrer">
      ...
      <div className="-mb-2 mt-3">
        {tweet.mediaDetails?.length ? (
          <div className={tweet.mediaDetails.length === 1 ? "" : "inline-grid grid-cols-2 gap-x-2 gap-y-2"}>
            {tweet.mediaDetails?.map((media) => (
              <a key={media.media_url_https} href={tweet.url} target="_blank" rel="noreferrer">
