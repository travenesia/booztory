...
export default function UpcomingPage() {
  ...
  useEffect(() => {
    ...
    observerRef.current = new IntersectionObserver(
      (entries) => {
        if (entries[0].isIntersecting && !isLoadingMore && visibleCount < otherItems.length) {
          loadMoreItems()
        }
      },
      { threshold: 0.8 },
  ...
  }, [isLoadingMore, loadMoreItems, visibleCount, otherItems.length])
      ...
      <section className="pt-6 pb-[80px] md:pb-[56px] px-6 max-w-[650px] mx-auto w-full">
        {allItems.length === 0 && !isLoadingMore && !isLoading ? (
          <div className="flex flex-col items-center justify-center py-16 text-center px-4">
            ...
            <p className="text-base font-bold text-gray-800 mb-1">Nothing in the queue yet</p>
            <p className="text-sm text-gray-500">Be the first — submit your content and claim the spotlight.</p>
          </div>
        ) : (
                ...
                <div className="overflow-hidden rounded-lg">
                  ...
                  >
                    {ownItems.map((content, i) => (
                      ...
                      >
                        <UpcomingCard content={content} isOwn />
                      </div>
                    ))}
                  </div>
                </div>
            ...
            {otherItems.length > 0 && (
              <div className="space-y-4 mt-2">
                {displayedContent.map((content, index) => (
                  ...
                  >
                    <UpcomingCard content={content} isOwn={false} />
                  </div>
                ))}
              </div>
            )}
        ...
        {visibleCount < otherItems.length && !isLoadingMore && (
          <div ref={loadMoreRef} style={{ height: "20px", margin: "10px 0" }} />
        )}
        {displayedContent.length > 0 && visibleCount >= otherItems.length && !isLoadingMore && (
