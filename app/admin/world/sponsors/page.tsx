...
import { usePriceTiersWorld } from "@/hooks/usePriceTiersWorld"
...
function parseAdContent(raw: string): Record<string, string> {
  try { const p = JSON.parse(raw); if (typeof p === "object" && p !== null) return p } catch {}
  return {}
}
...
function ApplicationRow({ appId, onAction }: { appId: number; onAction: () => void }) {
  ...
  const durationLabel = Number(duration) < 86400 ? `${Math.round(Number(duration) / 3600)}h` : `${Math.round(Number(duration) / 86400)}d`
  ...
  async function act(fn: "acceptApplication" | "rejectApplication") {
    setIsActing(true)
    try {
      const tx = await writeContractAsync({ address: WORLD_RAFFLE_ADDRESS, abi: WORLD_RAFFLE_ABI, functionName: fn, args: [BigInt(appId)], chainId: WORLD_CHAIN.id })
      ...
      if (!msg.toLowerCase().includes("rejected")) toast({ title: "Failed", variant: "destructive" })
    ...
    <div className="rounded-xl border border-gray-200 bg-white overflow-hidden">
      <div className="flex items-start justify-between gap-3 px-4 pt-4 pb-3">
        <div className="flex items-center flex-wrap gap-1.5">
          ...
          <span className={cn("text-xs font-semibold px-2 py-0.5 rounded-full border", statusInfo.color)}>{statusInfo.label}</span>
        ...
        <div className="text-right flex-shrink-0">
          ...
          {acceptedDate && <p className="text-[10px] text-green-600 mt-0.5">Accepted {acceptedDate}</p>}
        </div>
      </div>
      ...
      <div className="px-4 py-3 space-y-3">
        <div>
          ...
          <button onClick={() => { navigator.clipboard.writeText(sponsor); setCopied(true); setTimeout(() => setCopied(false), 1500) }}
            className="flex items-center gap-1.5 group p-0" title={copied ? "Copied!" : "Copy address"}>
            ...
            {copied ? <Check size={11} className="text-green-500" /> : <Copy size={11} className="text-gray-300 group-hover:text-gray-500 transition-colors" />}
          </button>
        </div>
        {parsed.sponsorName && <div><p className={L}>Sponsor Name</p><p className="text-sm font-semibold text-gray-800">{parsed.sponsorName}</p></div>}
        {adType === "image" && parsed.imageUrl && (
          <>
            <div><p className={L}>Image URL</p><p className="text-xs text-gray-700 truncate font-mono bg-gray-50 px-2 py-1 rounded">{parsed.imageUrl}</p></div>
            {parsed.ratio   && <div><p className={L}>Aspect Ratio</p><p className="text-xs text-gray-700">{parsed.ratio}</p></div>}
            {parsed.tagline && <div><p className={L}>Tagline</p><p className="text-xs text-gray-700">{parsed.tagline}</p></div>}
          </>
        )}
        {adType === "embed" && parsed.embedUrl && (
          <>
            <div><p className={L}>Embed URL</p><p className="text-xs text-gray-700 truncate font-mono bg-gray-50 px-2 py-1 rounded">{parsed.embedUrl}</p></div>
            {parsed.tagline && <div><p className={L}>Tagline</p><p className="text-xs text-gray-700">{parsed.tagline}</p></div>}
          </>
        )}
        {adType === "text" && parsed.text && <div><p className={L}>Ad Text</p><p className="text-xs text-gray-700 leading-relaxed">{renderFormattedText(parsed.text)}</p></div>}
...
function PriceTiersSection() {
  const { tiers, loading, refetch } = usePriceTiersWorld()
  const { toast } = useToast()
  const { writeContractAsync } = useWriteContract()
  ...
  async function handleSet() {
    const hours = parseFloat(ptHours), minPrize = parseFloat(ptMinPrize), fee = parseFloat(ptFee)
    if (!hours || hours <= 0 || !minPrize || minPrize <= 0 || isNaN(fee) || fee < 0) return
    setIsSaving(true)
    try {
      const tx = await writeContractAsync({
        address: WORLD_RAFFLE_ADDRESS, abi: WORLD_RAFFLE_ABI, functionName: "setPriceTier",
  ...
  async function handleRemove(seconds: number) {
    ...
    try {
      const tx = await writeContractAsync({
        address: WORLD_RAFFLE_ADDRESS, abi: WORLD_RAFFLE_ABI, functionName: "setPriceTier",
        args: [BigInt(seconds), 0n, 0n], chainId: WORLD_CHAIN.id,
      })
      await waitForTransactionReceipt(wagmiConfig, { hash: tx, chainId: WORLD_CHAIN.id })
      refetch(); toast({ title: "Tier Removed" })
    } catch (e) {
      const msg = e instanceof Error ? e.message : ""
      if (!msg.toLowerCase().includes("rejected")) toast({ title: "Failed", variant: "destructive" })
    ...
    <div className="rounded-xl border bg-white p-5 space-y-4">
      ...
      <div className="rounded-xl border overflow-hidden text-xs">
        <div className="grid grid-cols-[1.4fr_1fr_1fr_1fr_28px] px-3 py-2 text-gray-400 border-b font-semibold bg-gray-50">
          <span>Duration</span><span className="text-right">Prize</span><span className="text-right">Fee</span><span className="text-right">Total</span><span />
        </div>
        {loading ? (
          ...
          const prize = Number(tier.minPrize) / 1_000_000, fee = Number(tier.fee) / 1_000_000
          return (
            <div key={tier.seconds} className="grid grid-cols-[1.4fr_1fr_1fr_1fr_28px] items-center px-3 py-2.5 border-b last:border-0 text-gray-700">
              ...
              <span className="text-right font-bold tabular-nums text-blue-600">${(prize + fee).toFixed(2)}</span>
              <span className="flex justify-end">
                <button onClick={() => handleRemove(tier.seconds)} disabled={removingSeconds === tier.seconds}
      ...
      <button onClick={handleSet} disabled={isSaving || !ptHours || !ptMinPrize || parseFloat(ptMinPrize) <= 0}
        className="w-full bg-blue-600 text-white text-sm font-bold py-2.5 rounded-lg hover:bg-blue-700 disabled:opacity-50 transition-colors">
...
export default function WorldSponsorsPage() {
  const [page, setPage] = useState(0)

  const { data: nextAppIdRaw, refetch: refetchAppCount } = useReadContract({
    address: WORLD_RAFFLE_ADDRESS, abi: WORLD_RAFFLE_ABI, functionName: "nextApplicationId",
    chainId: WORLD_CHAIN.id, query: { refetchInterval: 30_000 },
  })
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-0.5">Review applications and manage pricing tiers on World Chain.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <h2 className="text-sm font-semibold text-gray-700 uppercase tracking-wide">Applications ({appCount})</h2>
            {pageCount > 1 && <span className="text-xs text-muted-foreground">Page {page + 1} / {pageCount}</span>}
          </div>

          {appCount === 0 ? (
            <div className="rounded-xl border bg-white px-4 py-8 text-center text-sm text-muted-foreground">No applications yet.</div>
          ) : (
            <>
              <div className="space-y-3">
                {pageIds.map(id => <ApplicationRow key={id} appId={id} onAction={() => { refetchAppCount(); setPage(0) }} />)}
              </div>
              {pageCount > 1 && (
                <div className="flex items-center justify-between pt-1">
                  <button onClick={() => setPage(p => Math.max(0, p - 1))} disabled={page === 0}
                    className="text-xs font-semibold px-3 py-1.5 rounded-lg border border-gray-200 hover:bg-gray-50 disabled:opacity-40 transition-colors">← Prev</button>
                  <div className="flex gap-1">
                    {Array.from({ length: pageCount }, (_, i) => (
                      <button key={i} onClick={() => setPage(i)}
                        className={`w-7 h-7 text-xs font-bold rounded-lg transition-colors ${i === page ? "bg-blue-600 text-white" : "border border-gray-200 hover:bg-gray-50 text-gray-600"}`}>
                        {i + 1}
                      </button>
                    ))}
                  </div>
                  <button onClick={() => setPage(p => Math.min(pageCount - 1, p + 1))} disabled={page === pageCount - 1}
                    className="text-xs font-semibold px-3 py-1.5 rounded-lg border border-gray-200 hover:bg-gray-50 disabled:opacity-40 transition-colors">Next →</button>
                </div>
              )}
            </>
          )}
